...
def semestres():
    if request.method == 'POST':
        ...
        curl.execute("SELECT * FROM usuario WHERE cui=%s",[cui])
        user = curl.fetchone()
        curl.close()
        ...
        if not user:
            ...
            if password == user["contrasenha"].encode('utf-8'):
                session['cui'] = user['cui']
                return render_template("semes.html")
            else:
                return render_template("error.html")
                #return "Error al ingresar la contraseña o CUI"
'''
...
@app.route('/Form',methods = ['POST','GET'])#nos enviara algo cada vez que entremos a nuestra pagina principal para evitar el not Found
def Form():
